    ...
    match action {
        ...
        PluginCommands::Install {
            ...
            println!(
                "{}",
                format!(
                    "Installing plugin '{}' to '{}' (software: {}, Minecraft: {})...",
                    project_id,
                    server_path.display(),
                    s.software,
                    s.version
            ...
            let dest = pm
                .install_from_modrinth_compatible(&server_path, &project_id, &loaders, &gvs)
                .await?;
            ...
            if let Ok(manifest) = inspect_jar_manifest(&dest) {
                ...
                if resolve_deps && !manifest.dependencies.is_empty() {
                    println!(
                        "{}",
                        "Checking and resolving plugin dependencies...".cyan().dimmed()
                    );
                    ...
                    for resolved in dep_res.resolved {
                        ...
                        if let Ok(dep_dest) = pm
                            .install_from_modrinth(&server_path, &resolved.project_id)
                            .await
                        {
                            println!(
        ...
        PluginCommands::Update { server, check, yes } => {
            let server_path = paths.resolve_server_path(None, Some(&server), true)?;
            let registry = ServersRegistry::load(paths)?;
            ...
            println!(
                "{}",
                format!(
                    "Checking for plugin updates on server '{}' ({} {})...",
            ...
            let candidates: Vec<_> = candidates
                .into_iter()
                .filter(|c| c.file_path.starts_with(&plugins_dir))
                .collect();

            if candidates.is_empty() {
                println!(
                    "{}",
                    "No installed plugins found or no upstream update hashes matched.".yellow()
                );
                return Ok(());
            }
            ...
            if updates_available.is_empty() {
                println!("{}", "\nAll plugins are up to date!".green().bold());
                return Ok(());
            }
            ...
            if check {
                println!(
                    "Run 'craft plugin update {}' to apply all updates with atomic rollback protection.",
                    s.name
                );
                return Ok(());
            }
            ...
            println!("{}", "\n[SUCCESS] Plugin update process complete.".green().bold());
        }
        PluginCommands::Inspect { file } => {
            if !file.exists() {
        ...
        PluginCommands::Doctor { server } => {
            ...
            println!(
                "{}",
                format!(
                    "Running plugin diagnostic audit for server '{}' ({} {})...",
                    s.name, s.software, s.version
                )
                .bold()
                .cyan()
            ...
            if !plugins_dir.exists() {
                println!("{}", "No 'plugins' directory found on server.".yellow());
                return Ok(());
            }
            ...
            if manifests.is_empty() {
                println!("{}", "No plugin JAR files found to audit.".yellow());
                return Ok(());
            }

            println!("Discovered {} installed plugin JAR(s).\n", manifests.len());
            ...
            for (_path, m) in &manifests {
                ...
                if *name_counts.get(&m.id_or_name.to_lowercase()).unwrap_or(&0) > 1 {
                    issues.push("Duplicate plugin name detected".to_string());
                    total_errors += 1;
                }
                ...
                for dep in &m.dependencies {
                    if dep.required {
                        ...
                        if lower != "minecraft" && lower != "java" && !installed_names.contains(&lower) {
                ...
                } else if !report.is_compatible || !issues.iter().all(|i| i.contains("Warning") || i.contains("may not satisfy")) {
            ...
            if total_errors == 0 && total_warnings == 0 {
                println!("{}", "[AUDIT PASSED] All plugins are healthy, compatible, and dependencies are satisfied.".green().bold());
            } else {
                println!(
