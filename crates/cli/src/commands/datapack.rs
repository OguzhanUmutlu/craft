...
pub async fn handle_datapack(action: DatapackCommands, paths: &CraftPaths) -> Result<()> {
    ...
    match action {
        DatapackCommands::Search { query } => {
            println!(
                "{}",
                format!("Searching datapacks for '{}' on Modrinth...", query).cyan()
            );
        ...
        DatapackCommands::Install {
            ...
            let dest = pm
                .install_datapack_from_modrinth(&server_path, &project_id, &target_world)
                .await?;
            println!(
                "{}",
                format!(
                    "[OK] Successfully installed datapack to '{}'!",
                    dest.display()
                )
                .green()
                .bold()
        ...
        DatapackCommands::List { server, world } => {
            ...
            if !caps.datapacks {
                println!(
                    "{}",
                    format!(
                        "[NOTE] Server '{}' (software: {}) does not support datapacks.",
                        s.name, s.software
                    )
                    .yellow()
                );
            ...
            if installed.is_empty() {
                println!(
                    "{}",
                    format!("No datapacks installed in '{}/datapacks'.", target_world).yellow()
                );
                return Ok(());
            }
            ...
            println!(
                "{}",
                format!(
                    "Installed datapacks in '{}' (world: {}, total: {}):",
                    s.name,
                    target_world,
                    installed.len()
                )
        ...
        DatapackCommands::Remove {
            ...
            println!(
                "{}",
                format!("[OK] Removed datapack '{}'.", file_to_delete.display())
                    .green()
                    .bold()
            );
        }
