...
impl KeyMap {
    ...
    #[allow(dead_code)]
    pub fn with_custom(mut self, code: KeyCode, modifiers: KeyModifiers, action: KeyAction) -> Self {
    ...
    pub fn resolve(&self, event: &KeyEvent) -> KeyAction {
        ...
        if (event.modifiers.contains(KeyModifiers::CONTROL)
            && (event.code == KeyCode::Char('c') || event.code == KeyCode::Char('C') || event.code == KeyCode::Char('d')))
        ...
        if self.allow_quit_on_q && event.modifiers.is_empty() && (event.code == KeyCode::Char('q') || event.code == KeyCode::Char('Q')) {
            return KeyAction::Quit;
        }
    ...
    pub fn footer_help_text(&self, mode: KeyHelpMode) -> String {
        ...
        match mode {
            KeyHelpMode::Menu => {
                format!("[↑/↓/j/k] Move  |  [Enter/→] Select  |  [Esc/←] Back{}", q_part)
            }
            KeyHelpMode::MenuWithToggle => {
                format!("[↑/↓] Move  |  [Space] Toggle  |  [Enter] Confirm  |  [Esc] Cancel{}", q_part)
            }
            KeyHelpMode::Input => {
            ...
            KeyHelpMode::Waiting => {
                "[Esc] Cancel  |  [Ctrl+C] Abort".to_string()
            }
            KeyHelpMode::Confirm => {
                "[←/→/Tab] Select  |  [Enter/y/n] Confirm  |  [Esc] Cancel".to_string()
            }
            KeyHelpMode::Table => {
                format!("[↑/↓] Select Row  |  [PgUp/PgDn] Page  |  [Enter] Open  |  [Esc] Back{}", q_part)
            }
        }
    }
}
