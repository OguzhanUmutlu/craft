    ...
    pub fn run(&self) -> Result<ConfirmOutcome> {
        ...
        let res = (|| -> Result<ConfirmOutcome> {
            loop {
                ...
                let clean_len = strip_ansi(&raw_combined).chars().count();
                let pad_left = inner_width.saturating_sub(clean_len) / 2;
                let centered_btn_row = format!("{}{}", " ".repeat(pad_left), raw_combined);

                frame.row(centered_btn_row);
