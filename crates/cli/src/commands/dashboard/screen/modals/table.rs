    ...
    pub fn run(&self, selected_idx: &mut usize) -> Result<TableOutcome> {
        ...
        let res = (|| -> Result<TableOutcome> {
            loop {
                ...
                // 2. Data Rows
                if scroll_offset > 0 {
                    frame.row(format!("[▲ {} rows above]", scroll_offset).dimmed().to_string());
                }
                ...
                for (local_i, row) in self.rows[scroll_offset..end_idx].iter().enumerate() {
                    ...
                    for (col_i, cell) in row.iter().enumerate() {
                        ...
                        let align_right = self.columns.get(col_i).map(|c| c.align_right).unwrap_or(false);
                        ...
                        let formatted_cell = if cell_len >= col_w {
                            ...
                            craft_core::truncate_ellipsis(&clean_cell, keep)
                        } else if align_right {
                ...
                if end_idx < self.rows.len() {
                    frame.row(format!("[▼ {} rows below]", self.rows.len() - end_idx).dimmed().to_string());
                }

                let help = self.footer_help.clone().unwrap_or_else(|| {
                    self.keymap.footer_help_text(KeyHelpMode::Table)
                });
                frame.footer(help);
                ...
                match event::read()? {
                    ...
                    Event::Key(key) => {
                        ...
                        match action {
                            ...
                            KeyAction::PageDown => {
                                *selected_idx = (*selected_idx + viewport_size).min(self.rows.len().saturating_sub(1));
                            }
                            KeyAction::Home => {
                                *selected_idx = 0;
                            }
