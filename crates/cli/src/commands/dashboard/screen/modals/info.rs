    ...
    pub fn run(&self) -> Result<()> {
        ...
        let res = (|| -> Result<()> {
            loop {
                ...
                if scroll_offset > 0 {
                    frame.row(format!("[▲ {} lines above]", scroll_offset).dimmed().to_string());
                }
                ...
                if end_idx < self.lines.len() {
                    frame.row(format!("[▼ {} lines below]", self.lines.len() - end_idx).dimmed().to_string());
                }
                ...
                match event::read()? {
                    ...
                    Event::Key(key) => {
                        ...
                        match action {
                            ...
                            KeyAction::PageDown => {
                                scroll_offset = (scroll_offset + viewport_size).min(self.lines.len().saturating_sub(viewport_size));
                            }
                            KeyAction::Home => {
                                scroll_offset = 0;
                            }
                            ...
                            KeyAction::Submit | KeyAction::Cancel | KeyAction::Toggle | KeyAction::Left | KeyAction::Right => {
                                break;
                            }
                            _ => {}
                        }
