...
| **`craft-remote`** | [`crates/remote/`](../crates/remote) | Multi-node SSH connection pooling, SFTP bidirectional synchronization, interactive remote PTY proxy, tri-platform remote host bootstrapping (Linux, macOS, Windows), federated log search query dispatch across remote nodes (`RemoteCraftClient::search_remote_logs`), and federated workload forecast and cost report queries (`RemoteCraftClient::get_remote_forecast`, `get_remote_cost_report`). |
| **`craft-scripting`** | [`crates/scripting/`](../crates/scripting) | Embedded Lua 5.4 engine (`mlua`) with sandboxed execution, deadline instruction hooks, `craft.*` stdlib (fs, servers, backup, net, http, audit, properties, exec), event-driven lifecycle hook bus (`HookBus`) supporting rollout lifecycle events, incident alerts, and workload forecasting surge events (`LifecycleEvent::WorkloadSurgePredicted`, `LifecycleEvent::CostOptimizationApplied`, `LifecycleEvent::ProactiveWakeTriggered`), and declarative software package definitions. |
...
| **`craft-cli`** | [`crates/cli/`](../crates/cli) | Main CLI entrypoint, 54 command handlers including AI workload forecasting and cost optimization (`craft forecast show`, `craft forecast cost`, `craft forecast schedule`, `craft forecast optimize`), log indexing & forensics (`craft log search`, `craft log forensics`, `craft log incidents`, `craft log index`), cluster fleet orchestration (`craft cluster rollout`, `rollout-status`, `rollback`, `heal`, `fleet-status`), `user`, `audit`, `optimize`, `modpack`, `autoscale`, `hibernate`, `mesh`, `dr`, `ai` (`status`, `analyze`, `profile`, `remediate`, `policy`), `edge` (`status`, `add`, `rm`, `probe`, `sync-routing`, `handoff`, `chat`, `optimize`), `profile` (`tick`, `packets`, `histogram`, `overview`), full-screen interactive TUI dashboard with ModalX centered Workload Forecasting & Cost Optimizer panel, virtual scrolling console with 16 KB chunked seek reader and inside-the-box anchored command prompt. |
