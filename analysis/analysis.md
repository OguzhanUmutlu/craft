...
| **`craft-cli`** | [`crates/cli/`](../crates/cli) | Main CLI entrypoint, 38 command handlers including `user`, `audit`, `optimize`, `modpack`, `autoscale`, `hibernate`, `mesh`, `dr`, full-screen interactive TUI dashboard, virtual scrolling console with 16 KB chunked seek reader and inside-the-box anchored command prompt. |
